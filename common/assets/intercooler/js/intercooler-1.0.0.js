...
/**
 * This file has been updated (see git history). Please do not replace it!!!!!
 *
...
var Intercooler = Intercooler || (function() {
  ...
  function getParametersForElement(verb, elt, triggerOrigin) {
    ...
    var data = null;
    var parentForm = elt.closest('form');

    if (verb == 'POST' && parentForm.length > 0 && parentForm.attr('enctype') == 'multipart/form-data' && parentForm.find('input[type="file"]').length > 0) {
      ...
      if (verb != "GET" && elt.closest('form').length > 0) {
        ...
        var eltData = elt.serialize();
        if (eltData) data += "&" + eltData;
      }
    }
